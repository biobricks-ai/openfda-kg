...
def extract_relationships_from_row(row):
    ...
    if sponsor_name:
        ...
        triples.append(
            (app_uri, namespaces["SIO"]["000136"], sponsor_uri)
        )  # SIO:is sponsored by
        triples.append(
            (sponsor_uri, namespaces["rdf"]["type"], namespaces["SIO"]["000012"])
        )  # SIO:organization
        triples.append(
            (sponsor_uri, namespaces["rdfs"]["label"], Literal(sponsor_name))
        )
        triples.append(
            (sponsor_uri, namespaces["dcterms"]["title"], Literal(sponsor_name))
        )
    ...
    if openfda and isinstance(openfda, dict):
        ...
        for brand_name in brand_names:
            triples.append(
                (app_uri, namespaces["skos"]["prefLabel"], Literal(brand_name))
            )
            triples.append(
                (app_uri, namespaces["SIO"]["000008"], Literal(brand_name))
        ...
        for generic_name in generic_names:
            triples.append(
                (app_uri, namespaces["skos"]["altLabel"], Literal(generic_name))
            )
            triples.append(
                (app_uri, namespaces["biolink"]["name"], Literal(generic_name))
            )
        ...
        for mfg_name in manufacturer_names:
            ...
            triples.append(
                (app_uri, namespaces["RO"]["0002234"], mfg_uri)
            )  # RO:manufactured by
            triples.append(
                (mfg_uri, namespaces["rdf"]["type"], namespaces["SIO"]["000012"])
            )  # SIO:organization
        ...
        for ndc in package_ndcs:
            ...
            triples.append(
                (app_uri, namespaces["SIO"]["000671"], ndc_uri)
            )  # SIO:has identifier
            triples.append(
                (ndc_uri, namespaces["rdf"]["type"], namespaces["IAO"]["0000578"])
            )  # IAO:centrally registered identifier
            triples.append(
                (ndc_uri, namespaces["dcterms"]["type"], Literal("package_ndc"))
            )
        ...
        for ndc in product_ndcs:
            ...
            triples.append(
                (ndc_uri, namespaces["dcterms"]["type"], Literal("product_ndc"))
            )
        ...
        for unii in uniis:
            ...
            triples.append(
                (app_uri, namespaces["RO"]["0000057"], substance_uri)
        ...
        for rxcui in rxcuis:
            ...
            triples.append(
                (app_uri, namespaces["SIO"]["000671"], rxcui_uri)
            )  # SIO:has identifier
            triples.append(
                (rxcui_uri, namespaces["rdf"]["type"], namespaces["biolink"]["Drug"])
            )
        ...
        for spl_id in spl_set_ids:
            ...
            triples.append(
                (app_uri, namespaces["SIO"]["000068"], spl_uri)
            )  # SIO:has document
            triples.append(
                (spl_uri, namespaces["rdf"]["type"], namespaces["IAO"]["0000310"])
            )  # IAO:document
            triples.append(
                (
                    spl_uri,
        ...
        for route in routes:
            ...
            triples.append(
                (app_uri, namespaces["ExO"]["0000002"], route_uri)
            )  # ExO:has exposure route
            triples.append(
                (route_uri, namespaces["rdf"]["type"], namespaces["ExO"]["0000055"])
        ...
        for prod_type in product_types:
            triples.append(
                (app_uri, namespaces["SIO"]["000332"], Literal(prod_type))
        ...
        for substance_name in substance_names:
            ...
            triples.append(
                (app_uri, namespaces["RO"]["0000057"], substance_name_uri)
            )  # RO:has participant
            triples.append(
                (
                    substance_name_uri,
    ...
    if products is not None:
        ...
        for i, product in enumerate(products_list):
            if isinstance(product, dict):
                ...
                triples.append(
                    (app_uri, namespaces["RO"]["0000057"], product_uri)
                )  # RO:has participant
                triples.append(
                    (
                        product_uri,
                ...
                if product.get("dosage_form"):
                    ...
                    triples.append(
                        (product_uri, namespaces["SIO"]["000008"], dosage_uri)
                    )  # SIO:has attribute
                    triples.append(
                        (
                            dosage_uri,
                            namespaces["rdf"]["type"],
                            namespaces["SIO"]["000614"],
                        )
                    )  # SIO:dosage form
                    triples.append(
                        (
                ...
                if product.get("route"):
                    ...
                    triples.append(
                        (product_uri, namespaces["ExO"]["0000002"], route_uri)
                    )  # ExO:has exposure route
                    triples.append(
                        (
                            route_uri,
                            namespaces["rdf"]["type"],
                            namespaces["ExO"]["0000055"],
                        )
                    )  # ExO:route of administration
                    triples.append(
                        (
        ...
        if len(products_list) > 0:  # Only add if there are products
            triples.append(
                (app_uri, namespaces["rdf"]["type"], namespaces["BAO"]["0000040"])
            )  # BAO:bioassay
            triples.append(
                (app_uri, namespaces["dcterms"]["identifier"], Literal(app_number))
            )
            triples.append(
                (app_uri, namespaces["dcterms"]["created"], Literal("FDA Application"))
            )
            triples.append(
                (app_uri, namespaces["SIO"]["000628"], Literal("pharmaceutical"))
            )  # SIO:refers to
